const passworsValidate=(password)=>{
    ...
    if(password.length<8){
        passwordErrors.length='length must be greater than or equal to 8'
    }
    const regex = /^(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/;
    if(!regex.test(password)){
        passwordErrors.strength='password must contain an uppercase letter,a number and a special symbol'
    }
